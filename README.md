<h2>Hey 👋, I'm Ahmed Nasser</h2>

<p>
I'm a <strong>Flutter Developer</strong> with <strong>3+ years of experience</strong> building
<strong>production-ready Android & iOS applications</strong>.
I work mainly on real business apps including e-commerce, logistics, booking systems,
and multi-role platforms.
</p>

<p>
📍 Egypt &nbsp;•&nbsp;
📱 Flutter & Dart &nbsp;•&nbsp;
🚀 15+ published apps
</p>

<p>
  <a href="https://preview--ahmed-nasser.lovable.app/">
    <img src="https://img.shields.io/badge/Portfolio-Website-4E69C8?style=flat-square&logo=Firefox" />
  </a>
  <a href="https://github.com/AhmedNasserZakii">
    <img src="https://img.shields.io/badge/GitHub-AhmedNasserZakii-000?style=flat-square&logo=github" />
  </a>
  <a href="https://www.linkedin.com/in/AhmedNasserZakii/">
    <img src="https://img.shields.io/badge/LinkedIn-AhmedNasserZakii-0077B5?style=flat-square&logo=linkedin" />
  </a>
  <a href="mailto:Ahmednasserzakii@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact%20Me-D14836?style=flat-square&logo=gmail&logoColor=white" />
  </a>
</p>

<h2>⚡ A Few Quick Facts</h2>

<hr width="60%" align="left" />

<img
  align="right"
  src="https://media1.giphy.com/media/13HgwGsXF0aiGY/giphy.gif"
  width="320"
/>

<ul>
  <li>🛠 Currently working on multiple Flutter apps in production.</li>
  <li>🧠 Strong in Clean Architecture, MVVM, and SOLID.</li>
  <li>📦 State Management: BLoC, Cubit, Provider, Riverpod, GetX.</li>
  <li>💳 Payments: PayMob, Fawry.</li>
  <li>🔔 Notifications & Analytics: FCM, OneSignal, AppsFlyer, UXCam.</li>
  <li>👨‍💻 Most of my work is available on GitHub.</li>
</ul>
<h2>🚀 Featured Projects</h2>

<ul>
  <li>
    <strong>Smouha SC App</strong> – Official club app for booking services & activities.
  </li>
  <li>
    <strong>Storex Manager</strong> – Vendor & inventory management app (Provider + CI/CD).
  </li>
  <li>
    <strong>IGR Management</strong> – Multi-role system with dynamic permissions & dashboards.
  </li>
  <li>
    <strong>Yalla Beena</strong> – Services & product marketplace.
  </li>
  <li>
    <strong>Parking System</strong> – Real-time reservations using Socket.IO.
  </li>
</ul>

<p>
👉 <a href="https://preview--ahmed-nasser.lovable.app/">View full portfolio</a>
</p>

<h2>🛠 Tools & Technologies</h2>

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/flutter/flutter-original.svg" width="25" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dart/dart-original.svg" width="25" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/firebase/firebase-plain.svg" width="25" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="25" />
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="25" />
</p>

<!-- <img src="https://github-readme-stats.vercel.app/api?username=AhmedNasserZakii&show_icons=true&count_private=true" /> -->
